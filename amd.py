...
class KabumCrawler():
    ...
    driver.get("https://www.kabum.com.br/hardware/placa-de-video-vga/placa-de-video-amd?page_number=1")
    ...
    try:
        ...
        df_kabum = df_kabum.drop_duplicates()
        filename = 'KABUM - VGA_AMD {}.csv'.format(now_save)
        outdir = './tables'
        if not os.path.exists(outdir):
            os.mkdir(outdir)
        savecrawler = os.path.join(outdir,filename)

        df_kabum.to_csv(savecrawler)
